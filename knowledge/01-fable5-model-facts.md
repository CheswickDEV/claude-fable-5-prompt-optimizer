...
> Snapshot: June 9, 2026 (launch day of Claude Fable 5); last verified against the live docs: July 14, 2026 — updated entries are marked *[updated 2026-07-14]*. Every statement below is based exclusively on official Anthropic sources (anthropic.com, platform.claude.com, www-cdn.anthropic.com). Source codes refer to the legend below. This file is the factual backbone of the optimizer: when a question is not answered here or in the other knowledge files, the correct answer is "not documented."
...
| [StopReasons] | https://platform.claude.com/docs/en/build-with-claude/handling-stop-reasons *(added to the docs after June 9; splits stop-reason handling out of [Refusals])* |
| [FallbackCredit] | https://platform.claude.com/docs/en/build-with-claude/fallback-credit *(added after June 9: dedicated page for the prompt-cache refund on fallback retries)* |
...
| **Availability** | GA since June 9, 2026: Claude API, Claude Platform on AWS, Amazon Bedrock (via Claude in Amazon Bedrock, the Messages-API endpoint), Vertex AI, Microsoft Foundry; agent harnesses: Claude Code, Claude Managed Agents (only a model-name update needed there). *[updated 2026-07-14]* Access was temporarily restricted and has been restored; the intro page banner links Anthropic's statement (anthropic.com/news/redeploying-fable-5) | [News], [Intro], [Product], [Migration] |
...
| **Other launch features** | *[updated 2026-07-14]* Memory tool; tool-result clearing via context editing (beta `context-management-2025-06-27`); compaction; vision; **code execution**; **programmatic tool calling** (both now explicitly in the supported-features list) | [Intro] |
...
| **Structured outputs** | *[updated 2026-07-14]* **Generally available for Fable 5 and Mythos 5** on the Claude API and Google Cloud (not in the Bedrock GA list): JSON outputs via `output_config.format` and strict tool use via `strict: true`; no beta header (the former `output_format` beta parameter is deprecated). Incompatible with citations and message prefilling; schema-complexity limits apply; refusals and `max_tokens` cutoffs can override the schema | [StructOut] |
...
| **Knowledge cutoff** | *[updated 2026-07-14]* Now documented in the models-overview comparison table: reliable knowledge cutoff **Jan 2026**, training data cutoff **Jan 2026** (same as Opus 4.8) | [Models] |
...
## Not documented / open points (as of June 9, 2026; re-checked July 14, 2026)

When users ask about these, answer "not documented" and cite this list:

1. ~~Structured outputs / strict tool use~~ *[resolved 2026-07-14]*: now documented as GA for Fable 5 (`output_config.format`, `strict: true`) — see the model knowledge table; [StructOut].
2. ~~Knowledge cutoff~~ *[resolved 2026-07-14]*: now documented — reliable and training cutoff Jan 2026; [Models].
...
8. **Tool search, parallel tool calls:** documented only generically for "current models," no Fable-5-specific statements. *(Programmatic tool calling was removed from this item on 2026-07-14: it is now explicitly listed as a Fable 5 supported feature; [Intro].)*
